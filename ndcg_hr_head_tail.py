...
def evaluate_rec_scores(k, user_movie_dict, user_emb_dict, movie_emb_dict):
    ...
    for user_id in user_emb_dict.keys():
        ...
        top_k_movies = movie_ids_array[top_k_movies_index]
...
movie_id_cnt = {}
for movie_id in movie_ids:
    if movie_id not in movie_id_cnt.keys():
        movie_id_cnt[movie_id] = 0
    movie_id_cnt[movie_id] = movie_id_cnt[movie_id] + 1

print("movie id lens: ", len(movie_id_cnt.keys()))
movie_id_value = []
for movie_id in movie_id_cnt.keys():
    movie_id_value.append(movie_id_cnt[movie_id])
movie_id_value.sort()
# print("movie_id_value: ", movie_id_value)
cnt_num = movie_id_value[int(len(movie_id_value) * 0.999)]
print("cnt_num: ", cnt_num)
for movie_id in movie_id_cnt.keys():
    if movie_id_cnt[movie_id] >= cnt_num:
        head_movie_ids.append(movie_id)
    else:
        tail_movie_ids.append(movie_id)

print("head_movie_ids: ", len(head_movie_ids))
print("tail_movie_ids: ", len(tail_movie_ids))
...
for used_movie_id in [head_movie_ids, tail_movie_ids]:
    ...
    for user_id, movie_id, rating, user_emb, item_emb in zip(user_ids, movie_ids, ratings, user_embs, item_embs):
        if movie_id not in used_movie_id:
            continue
