...
from zoo.multi_task_base import MultiTaskBase
from zoo.util.sub_task_utils import get_sub_task_class
from zoo.zoo_constants import ZooConstants


class MultiTaskAssembler(MultiTaskBase):
    def __init__(self, config):
        logging.info('MultiTaskAssembler.super().__init__(config): {}'.format(config))
        # super(MultiTaskAssembler, self).__init__(config)
        self._config = config
        self._sub_tasks = self._get_sub_tasks()

    def assemble_model(self, input_embedding_list, treatment, label_input_dict, extra_input_dict):
        config = self._config

        label_mapping = self._config.label_mapping
        ...
        for task_name, label_value in label_mapping.items():
            with tf.variable_scope(task_name):
                ...
                for sub_task in self._sub_tasks:
                    with tf.variable_scope(sub_task.name()):
        ...
        with tf.name_scope('loss'):
            ...
            for task_name, task_info_dict in task_dict.items():
                ...
                if label_mapping[task_name] in label_input_dict.keys():
                    ...
                    label_input = tf.zeros([tf.shape(treatment)[0]], dtype=tf.float32)
                ...
                loss_detail_dict = {}
                for sub_task in self._sub_tasks:
                    sub_task_loss, sub_task_loss_detail_dict = sub_task.calc_loss(task_info_dict, label_input, sample_weights, task_name, label_input_dict)
        ...
        return loss, output_dict, locals()

    def metrics_to_show(self):
        metrics = []
        for task in self._sub_tasks:
            metrics.extend(task.metrics_to_show())
        return super(MultiTaskAssembler, self).metrics_to_show() \
               + metrics
    ...
    def metrics_to_outptut(self):
        ...
        return super(MultiTaskAssembler, self).metrics_to_outptut() + metrics
    ...
    def _get_sub_tasks(self):
        ...
        for sub_task_name in sub_task_names:
            ...
            logging.info('_get_sub_tasks, amount_buckets: {}'.format(self._config.amount_buckets))
