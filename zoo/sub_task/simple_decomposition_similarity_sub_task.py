...
from lib.similarity_measure import SimilarityMeasure
from sub_task.sub_task_base import SubTaskBase
from util.common_utils import calc_cosine_similarity
from zoo_constants import ZooConstants


class SimpleDecompositionSimilaritySubTask(SubTaskBase):
    ITEM_ATOM_REPR_ORTHOGONAL_COL = 'item_atom_repr_orthogonal_col'
    ITEM_SIDE_REPR_COL = 'item_side_repr_col'
    QUERY_SIDE_REPR_COL = 'query_side_repr_col'
    QUERY_ATOM_REPR_ORTHOGONAL_COL = 'query_atom_repr_orthogonal_col'
    ...
    def __init__(self, config):
        ...
        self._query_atom_repr_orthogonal_col = self._config.get(SimpleDecompositionSimilaritySubTask.QUERY_ATOM_REPR_ORTHOGONAL_COL, None)
        self._query_side_repr_col = self._config.get(SimpleDecompositionSimilaritySubTask.QUERY_SIDE_REPR_COL, None)
        self._item_atom_repr_orthogonal_col = self._config.get(SimpleDecompositionSimilaritySubTask.ITEM_ATOM_REPR_ORTHOGONAL_COL, None)
    ...
    def do_construct_model(self, input_embedding, treatment, extra_input_dict, task_info_dict=None):
        query_atom_repr_orthogonal, query_side_repr, item_atom_repr_orthogonal, item_side_repr = self._get_reprs(task_info_dict)

        side_ipw_like_weight = self._calc_ipw_like_weight(query_atom_repr_orthogonal, query_side_repr, item_atom_repr_orthogonal, item_side_repr)
        ...
        qi_score_logit_atom = calc_cosine_similarity(query_atom_repr_orthogonal, item_atom_repr_orthogonal) / self._cosine_sim_scale_tau
        ...
        # v1
        qi_score_logit = calc_cosine_similarity(tf.concat([query_atom_repr_orthogonal, query_side_repr], axis=1)
                                                , tf.concat([item_atom_repr_orthogonal, item_side_repr], axis=1)) / self._cosine_sim_scale_tau
    ...
    def do_metrics_to_show(self):
        return super().do_metrics_to_show() + ['qi_score_logit_atom', 'qi_score_logit_side', 'qi_score_logit', 'qi_score_atom',
                                               'qi_score_side', 'qi_score_side', 'qi_score', 'side_ipw_like_weight',
                                              ]
    ...
    def _get_reprs(self, task_info_dict):
        query_atom_repr_orthogonal = task_info_dict.get(self._query_atom_repr_orthogonal_col, None)
        query_side_repr = task_info_dict.get(self._query_side_repr_col, None)
        item_atom_repr_orthogonal = task_info_dict.get(self._item_atom_repr_orthogonal_col, None)
        item_side_repr = task_info_dict.get(self._item_side_repr_col, None)

        return query_atom_repr_orthogonal, query_side_repr, item_atom_repr_orthogonal, item_side_repr
